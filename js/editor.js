/* ============================================================
   ...
   with 'H', then click "Edit". Four modes:
   ...
   Drag on floor = new box · click = select · handles = resize ·
   Delete = remove. Everything autosaves to your browser; Export
   hands back the JSON.
   ============================================================ */
window.EDITOR = (function(){
  const HB = window.HITBOXES;
  let visible=false, editing=false;
  let room="lounge", mode="solid";   // 'solid' | 'overlay' | 'door' | 'spawn'
  let selected=null;                  // {list,index} | {list:'spawn'}
  let drag=null;
  let canvas=null;
  let el={};
  ...
  const ROOM_LABEL={lounge:"The Lounge",gym:"The Gym",game:"The Game Room",music:"The Music Studio"};
  const ROOM_KEYS=["lounge","gym","game","music"];
  const RECT_MODES={solid:1,overlay:1,door:1};

  // ---------- data helpers ----------
  function listFor(m){ m=m||mode;
    if(m==="solid")   return HB.solids(room);
    if(m==="overlay") return HB.depth(room);
    if(m==="door")    return HB.doors(room);
    return null; // spawn
  }
  function selRect(){
    if(!selected || selected.list==="spawn") return null;
    const L=listFor(selected.list); return L ? L[selected.index]||null : null;
  }
  function firstOtherRoom(){ return ROOM_KEYS.find(r=>r!==room) || "lounge"; }
  function save(){ HB.save(); }
  ...
  function toast(msg){
    ...
    if(!t){ t=document.createElement("div"); t.id="hbToast"; document.body.appendChild(t); }
  ...
  // ---------- panel ----------
  function buildPanel(){
    const p=document.createElement("div"); p.id="hbE";
    p.innerHTML=`
      <div class="hbE-bar">
    ...
    el={ panel:p, body:q("#hbeBody"), room:q("#hbeRoom"), edit:q("#hbeEdit"),
      seg:[...p.querySelectorAll(".hbE-seg button")],
      addRow:q("#hbeAddRow"), add:q("#hbeAdd"), del:q("#hbeDel"),
      x:q("#hbeX"), y:q("#hbeY"), wh:q("#hbeWH"), w:q("#hbeW"), h:q("#hbeH"),
      baseWrap:q("#hbeBaseWrap"), base:q("#hbeBase"),
      toWrap:q("#hbeToWrap"), to:q("#hbeTo"),
      faceWrap:q("#hbeFaceWrap"), face:q("#hbeFace"),
      hint:q("#hbeHint"), exp:q("#hbeExport"), reset:q("#hbeReset"), done:q("#hbeDone") };

    el.edit.onclick=()=>setEditing(true);
    el.done.onclick=()=>setEditing(false);
    el.seg.forEach(b=>b.onclick=()=>setMode(b.dataset.m));
    el.add.onclick=addBoxCentered;
    el.del.onclick=deleteSelected;
    el.reset.onclick=()=>{ if(confirm("Reset "+ROOM_LABEL[room]+" to defaults? (collision, depth, doors & spawn)")){ HB.resetRoom(room); selected=null; refresh(); toast("Room reset"); } };
    el.exp.onclick=exportData;
    ["x","y","w","h","base"].forEach(key=>{
      el[key].oninput=()=>{
        const v=Math.round(parseFloat(el[key].value)||0);
        if(mode==="spawn"){ const sp=HB.spawn(room); if(key==="x")sp.x=v; if(key==="y")sp.y=v; save(); return; }
    ...
    el.to.onchange=()=>{ const r=selRect(); if(r){ r.to=el.to.value; save(); } };
    el.face.onchange=()=>{ const sp=HB.spawn(room); sp.face=el.face.value; save(); };
  }
  ...
  function populateTargets(){
    ...
    ROOM_KEYS.filter(r=>r!==room).forEach(r=>{
      const o=document.createElement("option"); o.value=r; o.textContent=ROOM_LABEL[r]; el.to.appendChild(o);
    });
  }
  ...
  function refreshFields(){
    ...
    if(mode==="spawn"){
      ...
      el.x.value=Math.round(sp.x); el.y.value=Math.round(sp.y);
      el.face.value=sp.face||"down";
      return;
    }
    const r=selRect();
    if(!r){ ["x","y","w","h","base"].forEach(k=>el[k].value=""); return; }
    el.x.value=Math.round(r.x); el.y.value=Math.round(r.y);
    el.w.value=Math.round(r.w); el.h.value=Math.round(r.h);
    if(mode==="overlay") el.base.value=Math.round(r.baseY!=null?r.baseY:(r.y+r.h));
  ...
  function addBoxCentered(){
    ...
    const L=listFor(mode); const cx=480, cy=320;
    let box;
    ...
    L.push(box); selected={list:mode,index:L.length-1}; save(); refresh();
  }
  function deleteSelected(){
    if(mode==="spawn" || !selected || selected.list==="spawn") return;
    const L=listFor(selected.list); if(!L) return;
    L.splice(selected.index,1); selected=null; save(); refresh(); toast("Deleted");
  }
  function exportData(){
    const text=HB.exportJSON();
    try{ const blob=new Blob([text],{type:"application/json"});
      const url=URL.createObjectURL(blob); const a=document.createElement("a");
      a.href=url; a.download="ryanworld-hitboxes.json"; a.click();
  ...
  // ---------- visibility / mode ----------
  function setVisible(v, rm){ visible=v; if(rm) room=rm;
    if(!el.panel) return;
    el.panel.classList.toggle("show", v);
    if(!v) setEditing(false); else refresh();
  }
  function setRoom(rm){ room=rm; selected=null; if(visible) refresh(); }
  function setEditing(v){
    editing=v; if(v){ visible=true; if(window.GAME&&GAME.setHitboxes) GAME.setHitboxes(true); }
  ...
  function handleAt(wx,wy,cam){
    ...
    if(mode==="overlay" && Math.abs(wy-(r.baseY))<=hs && wx>=r.x-hs && wx<=r.x+r.w+hs) return {id:"base"};
    return null;
  }
  function rectAt(wx,wy){
    const L=listFor(mode); if(!L) return -1;
  ...
  function onDown(e){
    ...
    if(mode==="spawn"){
      const sp=HB.spawn(room);
      sp.x=Math.round(p.x); sp.y=Math.round(p.y);
      selected={list:"spawn"}; drag={type:"spawn"}; refreshFields(); return;
    }
    // 1) handle of current selection
    const H=handleAt(p.x,p.y,cam);
    if(H){ const r=selRect(); drag={type:H.id==="base"?"baseline":"resize",H,orig:{...r},start:p}; return; }
    // 2) existing rect
    const idx=rectAt(p.x,p.y);
    if(idx>=0){ selected={list:mode,index:idx};
      drag={type:"move",orig:{...listFor(mode)[idx]},start:p}; refreshFields(); return; }
    // 3) draw new
    ...
    L.push(box); selected={list:mode,index:L.length-1};
    drag={type:"draw",start:p}; refreshFields();
  }
  function onMove(e){
    if(!editing||!drag) return;
    const p=GAME.screenToWorld(e.clientX,e.clientY);
    if(drag.type==="spawn"){ const sp=HB.spawn(room); sp.x=Math.round(p.x); sp.y=Math.round(p.y); refreshFields(); return; }
    const r=selRect(); if(!r) return;
    const dx=p.x-drag.start.x, dy=p.y-drag.start.y;
    if(drag.type==="move"){ r.x=Math.round(drag.orig.x+dx); r.y=Math.round(drag.orig.y+dy);
      if(r.baseY!=null && drag.orig.baseY!=null) r.baseY=Math.round(drag.orig.baseY+dy); }
    else if(drag.type==="baseline"){ r.baseY=Math.round(Math.max(0,Math.min(640,p.y))); }
    else if(drag.type==="draw"){ r.x=Math.round(Math.min(drag.start.x,p.x)); r.y=Math.round(Math.min(drag.start.y,p.y));
      r.w=Math.round(Math.abs(dx)); r.h=Math.round(Math.abs(dy)); if(r.baseY!=null) r.baseY=r.y+r.h; }
    else if(drag.type==="resize"){ const o=drag.orig, H=drag.H;
      let x=o.x,y=o.y,w=o.w,h=o.h;
      if(H.l){ x=o.x+dx; w=o.w-dx; } if(H.r){ w=o.w+dx; }
      if(H.t){ y=o.y+dy; h=o.h-dy; } if(H.b){ h=o.h+dy; }
  ...
  function onUp(){
    if(!drag) return;
    if(drag.type==="draw"){ const r=selRect();
      if(r && (r.w<4||r.h<4)){ listFor(mode).pop(); selected=null; refreshFields(); } }
    drag=null; save();
  }
  ...
  function drawOverlay(ctx,cam){
    if(mode==="spawn"){
      const sp=HB.spawn(room); if(!sp) return;
      ctx.save(); ctx.strokeStyle="#fff"; ctx.lineWidth=2/cam.s; ctx.setLineDash([5/cam.s,4/cam.s]);
      ctx.beginPath(); ctx.arc(sp.x,sp.y,12/cam.s,0,7); ctx.stroke(); ctx.setLineDash([]); ctx.restore();
      return;
    }
    const r=selRect(); if(!r) return;
    ctx.save();
    ctx.lineWidth=2/cam.s; ctx.setLineDash([6/cam.s,4/cam.s]);
    ctx.strokeStyle="#ffffff"; ctx.strokeRect(r.x,r.y,r.w,r.h);
    ctx.setLineDash([]);
    const hs=4.5/cam.s;
    HANDLES.forEach(H=>{ const hx=r.x+(H.l?0:H.r?r.w:r.w/2), hy=r.y+(H.t?0:H.b?r.h:r.h/2);
      ctx.fillStyle="#fff"; ctx.strokeStyle="#222"; ctx.lineWidth=1/cam.s;
      ctx.fillRect(hx-hs,hy-hs,hs*2,hs*2); ctx.strokeRect(hx-hs,hy-hs,hs*2,hs*2); });
    if(mode==="overlay"){ ctx.strokeStyle="#ffd728"; ctx.lineWidth=2.5/cam.s;
      ctx.beginPath(); ctx.moveTo(r.x,r.baseY); ctx.lineTo(r.x+r.w,r.baseY); ctx.stroke();
      ctx.fillStyle="#ffd728"; ctx.fillRect(r.x+r.w/2-hs,r.baseY-hs,hs*2,hs*2); }
    ctx.restore();
  }
  ...
  function init(){
    ...
    window.addEventListener("keydown",(e)=>{
      ...
      if(e.key==="Escape"){ selected=null; refreshFields(); }
    });
  }
  if(document.readyState==="loading") document.addEventListener("DOMContentLoaded",init);
  else init();

  return { setVisible, setRoom, isEditing, drawOverlay,
