...
type Phase = "ready" | "play" | "gap" | "done";
interface Bag {
  ox: number;
  oy: number;
  vx: number;
...
let bagImg: HTMLImageElement | null = null;
let portT = 0;
let talkUntil = 0;
let bubble = "";
...
function say(t: string, d?: number): void {
  bubble = t;
  talkUntil = performance.now() + (d || 2200);
}

export function open(opts?: OpenOpts): void {
  onClose = opts?.onClose ?? null;
  openOverlay(shell());
  (document.getElementById("wkClose") as HTMLElement).onclick = close;
  (document.getElementById("wkStart") as HTMLElement).onclick = start;
  bagImg = new Image();
  bagImg.src = "assets/ui/heavybag.png";
  bctx = (document.getElementById("wkBag") as HTMLCanvasElement).getContext("2d");
...
function start(): void {
  ...
  for (let i = 0; i < TOTAL; i++) {
    ...
    const seq: Dir[] = [];
    for (let j = 0; j < len; j++) seq.push(DIRS[Math.floor(Math.random() * 4)]);
    combos.push(seq);
  }
  (document.getElementById("wkStart") as HTMLElement).style.display = "none";
...
function loadCombo(i: number): void {
  ci = i;
  mi = 0;
...
function renderCombo(): void {
  const wrap = document.getElementById("wkArrows");
  if (!wrap) return;
  if (phase !== "play" || !combos[ci]) {
    wrap.innerHTML = "";
    return;
  }
  wrap.innerHTML = combos[ci]
    .map((d, i) => {
      const cls = i < mi ? "done" : i === mi ? "active" : "";
      return `<span class="wk-arrow ${cls} a-${d}">${DIR_GLYPH[d]}</span>`;
    })
    .join("");
}

function hit(dir: Dir): void {
...
function comboDone(success: boolean): void {
  if (success) {
    ...
    document.getElementById("wkScore")!.textContent = String(score);
    document.getElementById("wkStreak")!.textContent = String(streak);
...
function comboFail(reason: "miss" | "timeout"): void {
  streak = 0;
  document.getElementById("wkStreak")!.textContent = "0";
...
function flashBox(kind: "ok" | "bad"): void {
  const b = document.querySelector(".wk-combo-box") as HTMLElement | null;
  if (!b) return;
...
function endRound(): void {
  ...
  const btn = document.getElementById("wkStart") as HTMLElement;
  btn.style.display = "";
  ...
  if (score > best) {
    ...
    line = pick(LINES.endMid);
  } else {
    line = pick(LINES.endLow);
  }
...
function shoveBag(dir: Dir): void {
...
function addSpark(dir: Dir): void {
  const cx = 110, cy = 150;
...
function popWord(): void {
  const stage = document.getElementById("wkLeft");
  if (!stage) return;
...
function drawBag(dt: number): void {
  if (!bctx) return;
  const W = 220, H = 260;
  ...
  bag.vx += -bag.ox * 0.22;
  bag.vx *= 0.88;
  bag.ox += bag.vx * dt * 60 * 0.4;
  bag.vy += -bag.oy * 0.22;
  bag.vy *= 0.88;
  bag.oy += bag.vy * dt * 60 * 0.4;
  bag.shake = Math.max(0, bag.shake - dt * 3);
  const ang = bag.ox * 0.01; // lean from horizontal push (around mount)
  ...
  bctx.beginPath();
  bctx.ellipse(110 + bag.ox * 0.4, 236, 38, 9, 0, 0, 7);
  bctx.fill();
  ...
  for (const s of sparks) {
    ...
    s.life -= dt * 2.2;
    if (s.life > 0) {
      bctx.fillStyle = `rgba(231,200,90,${s.life})`;
      const r = Math.ceil(s.life * 4);
      bctx.fillRect(s.x - r / 2, s.y - r / 2, r, r);
    }
  }
...
function wireInput(): void {
  ...
  document.querySelectorAll<HTMLElement>("#wkDpad button").forEach((b) => {
    b.onclick = () => hit(b.getAttribute("data-d") as Dir);
  });
}
...
function step(dt: number): void {
  try {
    ...
    if (phase === "play" && combos[ci]) {
      comboTime -= dt;
      const cd = document.getElementById("wkCd");
      if (cd) {
        cd.style.width = Math.max(0, (comboTime / comboMax) * 100) + "%";
        cd.style.background = comboTime < comboMax * 0.33 ? "#b5442f" : "linear-gradient(90deg,#c98a36,#e0a64a)";
      }
    ...
    const pc = document.getElementById("wkPort") as HTMLCanvasElement | null;
    if (pc) {
      const c = pc.getContext("2d");
      if (c) drawPortraitFrame(c, "port_Gojo", talking ? f : 0, 92, 92);
    }
    const bub = document.getElementById("wkBubble");
    if (bub) {
    ...
    console.error("workout loop", err);
  }
}
